...
class Trainer:
    ...
    def train(
        ...
        **kwargs,
        ...
        #################################################
        regular_loss = torch.zeros(24).to(self.args.device)
        ...
        for epoch in range(epochs_trained, num_train_epochs):
            ...
            for step, inputs in enumerate(epoch_iterator):
                ...
                if (
                    ...
                    with model.no_sync():
                        l1,l2 = self.training_step(model, inputs, epoch_num = epoch)
                        # tr_loss += self.training_step(model, inputs, epoch_num = epoch)
                        tr_loss += l1
                        regular_loss += l2
                ...
                if (step + 1) % self.args.gradient_accumulation_steps == 0 or (
                    ...
                    self._maybe_log_save_evaluate(tr_loss, model, trial, epoch,regular_loss)
    ...
    def _maybe_log_save_evaluate(self, tr_loss, model, trial, epoch , regular_loss):
        if self.control.should_log:
            ...
            for i in range(regular_loss.shape[0]):
                logs["reg_loss{}".format(i)] = round(regular_loss[i].item() / (self.state.global_step - self._globalstep_last_logged), 4)
    ...
    def training_step(self, model: nn.Module, inputs: Dict[str, Union[torch.Tensor, Any]], epoch_num = None) -> torch.Tensor:
        """
        ...
        if self.use_amp:
            with autocast():
                if model.module.config.if_wlora:
                    loss,all_wlora_losses = self.compute_loss(model, inputs)
                else:
            ...
            if model.module.roberta.config.if_wlora == 1 and epoch_num >= model.module.roberta.config.start_epoch:
                print("loss",loss) # 暂时把alpha去掉了
                # loss += model.module.roberta.config.kl_alpha * torch.sum(torch.stack(all_wlora_losses))
        ...
        print('loss',loss)
        print('all_wlora_losses',all_wlora_losses)

        return loss.detach(),torch.stack(all_wlora_losses).detach()
    ...
    def compute_loss(self, model, inputs, return_outputs=False):
        ...
        try:
            if model.module.config.if_wlora:
                outputs,all_wlora_losses = model(**inputs)
            else:
                outputs = model(**inputs)
        except:
            if model.config.if_wlora:
                outputs,all_wlora_losses = model(**inputs)
            else:
                outputs = model(**inputs)
        ...
        try:
            if model.module.config.if_wlora:# 过于丑陋，先这样用着
                return (loss, outputs,all_wlora_losses) if return_outputs else (loss,all_wlora_losses)
        except:
            if model.config.if_wlora and not model.roberta.encoder.layer[0].attention.self.value.merged: # 更加丑陋，后期要改的
                return (loss, outputs,all_wlora_losses) if return_outputs else (loss,all_wlora_losses)
